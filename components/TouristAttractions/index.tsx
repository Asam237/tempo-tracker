...
const TouristAttractions: React.FC<TouristAttractionsProps> = ({
  attractions,
  cityName,
}) => {
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span
                          className={`text-sm font-light ${getDistanceColor(
                            attraction.distance
                          )}`}
                        >
                          {attraction.distance}
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white/5 rounded-xl p-4 text-center border border-white/10">
            ...
            <p className="text-white/80 text-sm font-light">
              Note moyenne:{" "}
              {(
                attractions.reduce((acc, attr) => acc + attr.rating, 0) /
                attractions.length
              ).toFixed(1)}
            </p>
          </div>
          <div className="bg-white/5 rounded-xl p-4 text-center border border-white/10">
            <div className="text-2xl mb-2">🚶</div>
            <p className="text-white/80 text-sm font-light">À moins de 2km</p>
          </div>
        </div>
      )}
    </div>
